...
class AppStore extends Component {
  state = {searchInput: '', activeTab: tabsList[0].tabId}

  onSelect = uniqueId => {
    this.setState({activeTab: uniqueId})
  }
  ...
  render() {
    ...
    let getFilteredList
    if (searchInput === '') {
      getFilteredList = appsList.filter(each => each.category === activeTab)
    } else {
      getFilteredList = appsList.filter(
        each =>
      ...
      <div className="bg-cont">
        ...
        <ul className="app-cont">
          {getFilteredList.map(item => (
            <AppItem appDetails={item} key={item.appId} />
          ))}
        </ul>
      </div>
    )
